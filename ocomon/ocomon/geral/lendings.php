	...
	<div class="container-fluid">
				...
				<table id="emprestimos" class="stripe hover order-column row-border" border="0" cellspacing="0" width="100%">
					...
					<tbody>
						...
						foreach ($resultado->fetchall() as $row) {
							...
							// $material2 = (new \Html2Text\Html2Text($row['material']))->getText();
		...
		if (isset($_GET['action']) && $_GET['action'] == "excluir") {
			...
			if (!$erro) {
				...
				$lendingDate = dateDB($_POST['saida']);
				$returningDate = (!empty($_POST['volta'])) ? dateDB($_POST['volta'], 1) : null;
				...
				try {
					...
					$resultado->bindParam(':responsavel', $_SESSION['s_uid'], PDO::PARAM_INT);
					$resultado->bindParam(':data_empr', $lendingDate, PDO::PARAM_STR);
					$resultado->bindParam(':data_devol', $returningDate, PDO::PARAM_STR);
					$resultado->bindParam(':quem', $quem, PDO::PARAM_STR);
					$resultado->bindParam(':local', $local, PDO::PARAM_INT);
					$resultado->bindParam(':ramal', $ramal, PDO::PARAM_STR);
					$resultado->execute();
					$aviso .= TRANS('MSG_SUCCESS_INSERT');
				} catch (Exception $e) {
					$erro = true;
					$aviso .= "" . TRANS('MSG_ERR_SAVE_RECORD') . "<br>" . $query . "<br>" . $e->getMessage();
				}
			}
			...
			try {
				...
				$resultado2->bindParam(':material', $material, PDO::PARAM_STR);
				$resultado2->bindParam(':responsavel', $_SESSION['s_uid'], PDO::PARAM_INT);
				$resultado2->bindParam(':ramal', $ramal, PDO::PARAM_STR);
				$resultado2->bindParam(':local', $local, PDO::PARAM_INT);
				$resultado2->bindParam(':data_empr', $lendingDate, PDO::PARAM_STR);
				$resultado2->bindParam(':data_devol', $returningDate, PDO::PARAM_STR);
				$resultado2->bindParam(':quem', $quem, PDO::PARAM_STR);
				$resultado2->bindParam(':empr_id', $cod, PDO::PARAM_INT);
				$resultado2->execute();
